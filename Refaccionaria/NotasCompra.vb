...
Imports System.IO
Public Class NotasCompra
    Private Sub Notas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim nota As New ClaseNotasCompras()
        nota.PoblarDataGridNotasCompras(DGVNotasCompras)
        txtcdgCompra.Text = codigoCompra
        txtfecha.Text = nota.getFecha()
        txtcliente.Text = nota.getProveedor()
    ...
    Public Sub ExportarDatosPDF(ByVal document As Document)
        ...
        Dim datatable As New PdfPTable(DGVNotasCompras.ColumnCount)
        'Se asignan las propiedades del .PDF
        datatable.DefaultCell.Padding = 3
        Dim headerwidths As Single() = GetColumnasSize(DGVNotasCompras)
        datatable.SetWidths(headerwidths)
        datatable.WidthPercentage = 100
        datatable.DefaultCell.BorderWidth = datatable.DefaultCell.HorizontalAlignment = Element.ALIGN_LEFT
        'Se crea el encabezado en el PDF.
        Dim encabezado As New Paragraph("Compra NUMERO " & txtcdgCompra.Text & "", New Font(Font.Name = "Tahoma", 20, Font.Bold))
        ...
        Dim cliente As New Paragraph("Proveedor: " & txtcliente.Text & "", New Font(Font.Name = "Tahoma", 14, Font.Bold))
        Dim total As New Paragraph("Total: " & txttotal.Text & "", New Font(Font.Name = "Tahoma", 14, Font.Bold))
        'Se capturan los nombres de las columnas del DataGridView.
        For i As Integer = 0 To DGVNotasCompras.ColumnCount - 1
            datatable.AddCell(DGVNotasCompras.Columns(i).HeaderText)
        Next
        datatable.HeaderRows = 1
        datatable.DefaultCell.BorderWidth = 1  'Se generan las columnas del DataGridView.
        For i As Integer = 0 To DGVNotasCompras.RowCount - 2
            For j As Integer = 0 To DGVNotasCompras.ColumnCount - 1
                ...
                'Else
                Dim cell As New PdfPCell(New Paragraph(DGVNotasCompras(j, i).Value.ToString()))
                If (j = DGVNotasCompras.ColumnCount - 1 Or j = DGVNotasCompras.ColumnCount - 3) Then
